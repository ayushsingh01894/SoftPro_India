...
for result in results:
    print(f"  [{result.metadata['source']:8}] {result.page_content}")
print()

print("with scores (lower distance = closer):")
for result, score in store.similarity_search_with_score(query, k=2):
    print(f"  {score:.3f}  [{result.metadata['source']}]")
print()
